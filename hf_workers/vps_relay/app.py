...
def _yt_resolve_publish_schedule(youtube, privacy_status="unlisted", schedule_after_hours=0):
    ...
    daily_limit = int(_cfg("pipeline_youtube_daily_publish_limit", 3))
        ...
        if status == "done" and result:
            ...
            if job and job.get("book_id"):
                ...
                if project_flag:
                    ...
                    if book_row:
                        existing_flags = _shared_normalize_text_items(book_row.get("status") or "")
...
def _update_worker_stats(worker_id: str, worker_type: str, success: bool, duration_seconds: int):
    ...
    try:
        with _get_conn() as conn:
            with conn.cursor() as cur:
                _shared_upsert_worker_stats(cur, worker_id, worker_type, success, duration_seconds)
            conn.commit()
    except Exception as e:

...
def resolve_postgres_dsn() -> str:
    """从环境变量解析 PostgreSQL DSN，兼容 postgresql+psycopg:// 前缀。

    供 build_runtime_config、task_service._run_pipeline_in_thread、tests._build_test_config 共用。
    """
...
def build_runtime_config(channel_name: str, overrides: dict = None) -> dict:
    ...
    from ..config_schema import CHANNEL_SPECIFIC_KEYS, DEFAULT_CONFIG
    ...
    # 频道专属 Key 的默认值补全（如果频道配置里没有，用 DEFAULT_CONFIG 兜底）
    for key in CHANNEL_SPECIFIC_KEYS:
        if key not in config and key in DEFAULT_CONFIG:
            config[key] = coerce_value(key, DEFAULT_CONFIG[key])

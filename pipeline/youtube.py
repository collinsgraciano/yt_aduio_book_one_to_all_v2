...
from .runtime import log, write_json_file, read_json_file, extract_youtube_video_id as _extract_youtube_video_id
...
def _resolve_channel_proxy(channel_name: str) -> str:
    """从运行配置中读取本频道的 SOCKS5 代理地址。

    YOUTUBE_UPLOAD_PROXIES 是频道级配置，直接存储代理 URL（如 socks5://[ipv6]:1080）。
    返回代理 URL 字符串，未配置则返回空字符串。
    """
    return str(getattr(cfg, "YOUTUBE_UPLOAD_PROXIES", "") or "").strip()


def _build_youtube_with_proxy(credentials, channel_name: str):
    """构建 YouTube API 客户端，如果配置了频道级代理则通过代理连接。

    googleapiclient 的 build() 底层使用 httplib2.Http，
    通过注入带 SOCKS5 代理的 Http 对象实现。
    """
...
def authenticate_youtube_from_supabase(channel_name):
    ...
    try:
        ...
        youtube = _build_youtube_with_proxy(credentials, channel_name)

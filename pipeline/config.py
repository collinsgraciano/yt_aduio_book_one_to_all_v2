...
DEFAULT_RUNTIME_CONFIG = {
    ...
    "MIN_BOOK_DURATION_SECONDS": 1800,
    "SKIP_EXISTING": True,
    ...
    "STEREO_OFFSET": 0.0,
    "BGM_DUCKING_MODE": "sidechain",
    "BGM_BASE_GAIN_DB": -15,
    "BGM_DUCK_THRESHOLD_DB": -30,
    "BGM_DUCK_THRESHOLD_OFFSET_DB": -5,
    "BGM_DUCK_RATIO": 8,
    "BGM_DUCK_ATTACK_MS": 5,
    "BGM_DUCK_RELEASE_MS": 400,
    "BGM_INTRO_OUTRO_SECONDS": 3,
    "PIPELINE_TASK_ID": "",
    ...
    "TELEGRAM_API_BASE": "",
    "YOUTUBE_OAUTH_BASE": "",
    # 频道级 SOCKS5 代理（逗号分隔：频道名1=socks5://[ipv6]:port,频道名2=socks5://[ipv6]:port）
    "YOUTUBE_UPLOAD_PROXIES": "",
...
def normalize_runtime_source(source, default="database"):
    """将运行配置来源标准化为 'database' 或 'local'。"""
    if not source or not str(source).strip():
        return default
    source = str(source).strip().lower()
    if source in ("database", "db", "postgres", "postgresql", "supabase"):
        return "database"
    if source in ("local", "env", "environment"):
        return "local"
    return default
...
def collect_runtime_config_snapshot():
    return {
        ...
        "download_workers": getattr(sys.modules[__name__], "DOWNLOAD_WORKERS", 2),
        ...
        "deepfilter_workers": getattr(sys.modules[__name__], "DEEPFILTER_WORKERS", 1),

...
# ═══════════════════════════════════════════════════════════════
# 用法：
#   bash scripts/project_import.sh <迁移包路径>
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -h|--help)
            echo "用法: bash scripts/project_import.sh <迁移包路径> [--force]"
            echo ""
            echo "参数:"
...
if [ -z "$BUNDLE_FILE" ]; then
    error "必须指定迁移包路径"
    echo '  用法: bash scripts/project_import.sh migration_bundle_20260721_120000.tar.gz'
    exit 1
fi

...
if [ $# -lt 1 ]; then
    echo "用法: bash scripts/db_backup/db_restore.sh <备份文件路径>"
    echo "示例: bash scripts/db_backup/db_restore.sh backups/audiobook_backup_20260720_120000.sql.gz"
    exit 1
fi
...
echo "  建议先执行备份：bash scripts/db_backup/db_backup.sh"
echo ""
...
# 先断开所有连�?docker exec "$CONTAINER" psql -U "$PG_USER" -d postgres -c "

...
# 导出内容：
#   1. 数据库完整备份（表结构 + 全部数据）
#   2. .env 配置文件（SECRET_KEY、POSTGRES_PASSWORD 等）
#   3. relay-data 卷（HF 中继配置 relay_config.json）
#   4. output_data 卷（可选，生成的音频文件）
#
# 用法：
#   bash scripts/project_migration/project_export.sh                  # 不含输出文件
#   bash scripts/project_migration/project_export.sh --with-output     # 含输出文件
#   bash scripts/project_migration/project_export.sh /tmp              # 导出到指定目录
...
if [ "$WITH_OUTPUT" = true ]; then
    ...
    echo "  含音频输出: 否（使用 --with-output 启用）"
fi
echo "═══════════════════════════════════════════════════"
...
if [ -n "$RELAY_VOL" ]; then
    ...
    warn "未找到 relay-data 卷（容器 ${CONTAINER_RELAY} 可能未运行），跳过"
fi

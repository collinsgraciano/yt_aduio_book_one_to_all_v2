...
#
# 前置条件�?#   1. 已克隆项目仓库到目标 VPS（git clone�?#   2. 已安�?Docker + Docker Compose
#   3. 迁移包已传输到目�?VPS
#
# 警告：此脚本会删除目�?VPS 上的所有现有数据！
# ══════════════════════════════════════════════════════════════�?
set -euo pipefail
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -h|--help)
            echo "用法: bash scripts/project_migration/project_import.sh <迁移包路�? [--force]"
            echo ""
            echo "参数:"
            echo "  <迁移包路�?  migration_bundle_*.tar.gz 文件路径"
            echo "  --force       跳过确认提示"
            exit 0
            ;;
        *) BUNDLE_FILE="$1"; shift ;;
...
if [ -z "$BUNDLE_FILE" ]; then
    error "必须指定迁移包路�?
    echo '  用法: bash scripts/project_migration/project_import.sh migration_bundle_20260721_120000.tar.gz'
    exit 1
fi
...
if docker compose version >/dev/null 2>&1; then
    ...
    error "未找�?docker compose 命令，请安装 Docker Compose"
    exit 1
fi
...
if [ -f "${TMP_DIR}/manifest.txt" ]; then
    ...
    info "迁移包清�?"
    # 不显示密码行
    grep -v "POSTGRES_PASSWORD" "${TMP_DIR}/manifest.txt" | sed 's/^/  /'
fi
...
if [ -f "${TMP_DIR}/env_backup" ]; then
    if [ -f "${PROJECT_ROOT}/.env" ]; then
        ...
        info "已备份现�?.env �?.env.bak_${TIMESTAMP}"
    fi
    ...
    if [ -n "$BASE_URL" ]; then
        ...
        warn "�?VPS IP 已变更，迁移后需修改 .env 中的 BASE_URL"
    fi
else
    warn "迁移包中�?.env，将使用 .env.example"
    if [ ! -f "${PROJECT_ROOT}/.env" ]; then
        cp "${PROJECT_ROOT}/.env.example" "${PROJECT_ROOT}/.env"
        error "请编�?.env 配置后重新运�? nano .env"
        exit 1
    fi
fi
...
for i in $(seq 1 30); do
    ...
    if [ $i -eq 30 ]; then
        error "PostgreSQL 未就�?
        echo "  查看日志: $DC logs postgres"
        echo "  临时目录保留供调�? ${TMP_DIR}"
        exit 1
    fi
    sleep 2
done
...
# 停止 web �?vps-relay（释放数据库连接�?info "停止 Web 和中继服�?.."
...
if [ -f "${TMP_DIR}/relay_data.tar.gz" ]; then
    ...
    if [ -n "$RELAY_VOL" ]; then
        ...
        warn "未找�?relay-data 卷（容器 ${CONTAINER_RELAY} 不存在），跳�?
    fi
else
    info "迁移包中�?relay_data.tar.gz，跳�?
fi

# ─── 8. 恢复 output_data 卷（如果包含）───
echo ""
echo "[8/9] 恢复 output_data �?.."
if [ "$INCLUDE_OUTPUT" = true ] && [ -f "${TMP_DIR}/output_data.tar.gz" ]; then
    ...
    info "迁移包中�?output_data.tar.gz，跳�?
fi
...
echo ""
echo "  2. 更新 Google Cloud Console OAuth 重定�?URI:"
...
echo "     ${DC} restart web"
echo ""
echo "  4. 验证 YouTube 频道 OAuth 状�?
echo "     # �?Web 界面检查频道授权是否有�?
echo ""
echo "  ─── 服务地址 ───"
echo ""

...
# ═══════════════════════════════════════════════════════════════
# 用法：
#   bash scripts/project_migration/project_import.sh <迁移包路径>
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -h|--help)
            echo "用法: bash scripts/project_migration/project_import.sh <迁移包路径> [--force]"
            echo ""
            echo "参数:"
            echo "  <迁移包路径>  migration_bundle_*.tar.gz 文件路径"
            echo "  --force       跳过确认提示"
            exit 0
            ;;
        *) BUNDLE_FILE="$1"; shift ;;

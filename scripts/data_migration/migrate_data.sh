#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════�?# 数据迁移脚本 �?从旧 PostgreSQL 迁移核心表数据到本项�?# ══════════════════════════════════════════════════════════════�?# 用法�?#   bash scripts/data_migration/migrate_data.sh
#
# 环境变量（在 .env 或命令行设置）：
#   OLD_PG_HOST     旧数据库地址（默�?127.0.0.1�?#   OLD_PG_PORT     旧数据库端口（默�?5432�?#   OLD_PG_USER     旧数据库用户（默�?postgres�?#   OLD_PG_DB       旧数据库名（默认 audiobook�?#   OLD_PG_PASSWORD 旧数据库密码（必填）
#
# 示例�?#   OLD_PG_PASSWORD=old_pass bash scripts/data_migration/migrate_data.sh
#   OLD_PG_HOST=85.121.48.55 OLD_PG_PASSWORD=old_pass bash scripts/data_migration/migrate_data.sh
# ══════════════════════════════════════════════════════════════�?
set -euo pipefail
...
echo "  目标数据�? 容器 ${NEW_CONTAINER} / ${NEW_PG_DB} (用户: ${NEW_PG_USER})"
echo "  迁移�? ${CORE_TABLES[*]}"
echo ""
warn "仅迁移上�?${#CORE_TABLES[@]} 张核心表，其余表（global_settings, channels, audiobook_chapters 等）不会迁移"
echo ""

# ─── 检查旧数据库密�?───
if [ -z "$OLD_PG_PASSWORD" ]; then
    error "未设�?OLD_PG_PASSWORD"
    echo "  用法: OLD_PG_PASSWORD=your_password bash scripts/data_migration/migrate_data.sh"
    exit 1
fi
...
if PGPASSWORD="$OLD_PG_PASSWORD" psql -h "$OLD_PG_HOST" -p "$OLD_PG_PORT" -U "$OLD_PG_USER" -d "$OLD_PG_DB" -c "SELECT 1;" >/dev/null 2>&1; then
    ...
    echo "  请检�?OLD_PG_HOST / OLD_PG_PORT / OLD_PG_USER / OLD_PG_PASSWORD"
    exit 1
fi
...
for table in "${CORE_TABLES[@]}"; do
    ...
    if [ "$EXISTS" = "t" ]; then
        ...
        error "�?${table} 不存在，请确�?docker/init-db.sql 已执�?
        exit 1
    fi
done
ok "所有表结构验证通过"
...
for table in "${CORE_TABLES[@]}"; do
    ...
    if [ "$OLD_COUNT" = "$NEW_COUNT" ]; then
        ...
        warn "${table}: 行数不一�?(�? ${OLD_COUNT}, �? ${NEW_COUNT})"
        ALL_OK=false
    fi
done
...
if [ "$ALL_OK" = true ]; then
    ...
    warn "迁移完成，但部分表行数不一致，请检查日�?
fi
echo ""
echo "  下一�?"
echo "    1. �?Web 界面配置全局设置（MODELSCOPE_TOKEN、TG_BOT_TOKEN 等）"
echo "    2. 添加频道并完�?OAuth 授权"

#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════�?#  服务器端部署脚本
#  在服务器上手动运行：git pull �?智能构建 �?重启 �?健康检�?#
#  用法（SSH 登录服务器后）：
#    cd /root/audiobook
#    bash scripts/deploy/git-server-deploy.sh
# ══════════════════════════════════════════════════════════════�?
set -euo pipefail

SERVER_PATH="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "${SERVER_PATH}"

# ─── 检�?docker compose 命令（优�?v2，避�?snap 沙箱问题）───
if docker compose version >/dev/null 2>&1; then
    DC="docker compose"
elif docker-compose version >/dev/null 2>&1; then
    DC="docker-compose"
else
    echo "  [x] 未找�?docker compose 命令，请安装 Docker"
    exit 1
fi

echo "══════════════════════════════════════════════════════════�?
echo "  部署开�?�?$(date '+%Y-%m-%d %H:%M:%S')"
echo "  路径: ${SERVER_PATH}"
echo "══════════════════════════════════════════════════════════�?

# ─── 1. 拉取最新代�?───
echo "[1/4] git pull..."
OLD_SCRIPT_HASH=$(md5sum "${BASH_SOURCE[0]}" 2>/dev/null | awk '{print $1}' || echo "")
git pull
NEW_SCRIPT_HASH=$(md5sum "${BASH_SOURCE[0]}" 2>/dev/null | awk '{print $1}' || echo "")
echo "  当前版本: $(git rev-parse --short HEAD)"
# 如果脚本自身�?git pull 更新了，用新版本重新执行
if [ "$OLD_SCRIPT_HASH" != "$NEW_SCRIPT_HASH" ] && [ -z "${DEPLOY_SCRIPT_REEXECED:-}" ]; then
    echo "  > 部署脚本自身有更新，自动重新执行新版�?.."
    export DEPLOY_SCRIPT_REEXECED=1
    exec bash "${BASH_SOURCE[0]}" "$@"
fi
echo ""

# ─── 2. 检�?.env ───
echo "[2/4] 检�?.env..."
if [ ! -f .env ]; then
    if [ -f .env.example ]; then
        cp .env.example .env
        echo "  [!] 已从 .env.example 创建 .env，请编辑后重新运�?
        echo "      nano .env"
        exit 1
    else
        echo "  [x] .env 不存在，请手动创�?
        exit 1
    fi
else
    echo "  .env OK"
fi
echo ""

# ─── 2.5 预下�?DeepFilter 二进制（持久化到宿主机）───
echo "[2.5/4] 检�?DeepFilter 二进�?.."
DEEPFILTER_DIR="${SERVER_PATH}/data/deepfilter"
DEEPFILTER_BIN="deep-filter-0.5.6-x86_64-unknown-linux-musl"
DEEPFILTER_URL="https://github.com/Rikorose/DeepFilterNet/releases/download/v0.5.6/deep-filter-0.5.6-x86_64-unknown-linux-musl"

mkdir -p "$DEEPFILTER_DIR"
if [ -f "$DEEPFILTER_DIR/$DEEPFILTER_BIN" ]; then
    echo "  �?DeepFilter 已在宿主机缓存中"
else
    echo "  > 下载 DeepFilter �?$DEEPFILTER_DIR ..."
    if wget --tries=5 --timeout=30 --retry-connrefused \
        "$DEEPFILTER_URL" -O "$DEEPFILTER_DIR/$DEEPFILTER_BIN"; then
        chmod +x "$DEEPFILTER_DIR/$DEEPFILTER_BIN"
        echo "  �?DeepFilter 下载完成（后续重建镜像不再重复下载）"
    else
        echo "  [!] DeepFilter 下载失败，容器启动时会自动重�?
    fi
fi
echo ""

# ─── 2.6 预下�?BGM 音乐库（持久化到宿主机）───
echo "[2.6/4] 检�?BGM 音乐�?.."
MUSIC_DIR="${SERVER_PATH}/data/music"
MUSIC_ZIP_URL="https://huggingface.co/datasets/oooooo1323/cm/resolve/main/Parisian%20Breeze.zip"
MUSIC_ARCHIVE="${MUSIC_DIR}/Parisian_Breeze.zip"

mkdir -p "$MUSIC_DIR"
EXISTING_MUSIC=$(find "$MUSIC_DIR" -type f \( -iname "*.mp3" -o -iname "*.wav" -o -iname "*.flac" -o -iname "*.m4a" -o -iname "*.ogg" -o -iname "*.aac" -o -iname "*.wma" \) 2>/dev/null | wc -l)
if [ "$EXISTING_MUSIC" -gt 0 ]; then
    echo "  �?BGM 音乐库已�?${EXISTING_MUSIC} 个音频文�?
else
    echo "  > 下载 BGM 音乐包到 $MUSIC_DIR ..."
    if wget --tries=5 --timeout=60 --retry-connrefused \
        "$MUSIC_ZIP_URL" -O "$MUSIC_ARCHIVE"; then
        echo "  > 解压音乐�?.."
        unzip -o -q "$MUSIC_ARCHIVE" -d "$MUSIC_DIR" 2>/dev/null || python3 -c "
import zipfile, sys
with zipfile.ZipFile(sys.argv[1]) as z:
    z.extractall(sys.argv[2])
" "$MUSIC_ARCHIVE" "$MUSIC_DIR"
        rm -f "$MUSIC_ARCHIVE"
        MUSIC_COUNT=$(find "$MUSIC_DIR" -type f \( -iname "*.mp3" -o -iname "*.wav" -o -iname "*.flac" -o -iname "*.m4a" -o -iname "*.ogg" -o -iname "*.aac" -o -iname "*.wma" \) 2>/dev/null | wc -l)
        echo "  �?BGM 音乐库下载完成，�?${MUSIC_COUNT} 个音频文�?
    else
        echo "  [!] BGM 音乐下载失败，容器启动时会自动重�?
    fi
fi
echo ""

# ─── 2.7 检�?5432 端口占用（关闭系统自�?PostgreSQL / 残留容器）───
echo "[2.7/4] 检�?5432 端口占用..."
PORT_5432_OCCUPIED=$(ss -tlnp 2>/dev/null | grep ':5432 ' || echo "")
if [ -n "$PORT_5432_OCCUPIED" ]; then
    PORT_5432_PID=$(echo "$PORT_5432_OCCUPIED" | head -1 | sed -n 's/.*pid=\([0-9]*\).*/\1/p')
    PORT_5432_PROC=$(ps -p "${PORT_5432_PID}" -o comm= 2>/dev/null || echo "unknown")
    echo "  [!] 端口 5432 被占�? PID=${PORT_5432_PID} (${PORT_5432_PROC})"

    if [ "$PORT_5432_PROC" = "docker-proxy" ] || [ "$PORT_5432_PROC" = "docker" ]; then
        # ─── 情况 A: Docker 容器占用了端�?───
        echo "  > 检测到 Docker 容器占用端口"
        OLD_PG_CONTAINER=$(docker ps --filter "publish=5432" --format '{{.Names}}' 2>/dev/null | head -1 || echo "")
        if [ -n "$OLD_PG_CONTAINER" ]; then
            echo "  > 停止残留容器: ${OLD_PG_CONTAINER}"
            docker stop "$OLD_PG_CONTAINER" 2>/dev/null && echo "  �?已停�?${OLD_PG_CONTAINER}"
            docker rm "$OLD_PG_CONTAINER" 2>/dev/null
        fi
    else
        # ─── 情况 B: 系统自带 PostgreSQL 占用了端�?───
        PG_SERVICE=$(systemctl list-units --type=service --state=running 2>/dev/null \
            | grep -oE 'postgresql[^ ]*' | head -1 || echo "")
        if [ -n "$PG_SERVICE" ]; then
            echo "  > 停止系统 PostgreSQL 服务: ${PG_SERVICE}"
            systemctl stop "$PG_SERVICE" 2>/dev/null && echo "  �?已停�?${PG_SERVICE}"
            systemctl disable "$PG_SERVICE" 2>/dev/null && echo "  �?已禁用开机自�?${PG_SERVICE}"
        else
            echo "  [!] 未找�?PostgreSQL systemd 服务，尝试直接终止进�?.."
            kill "$PORT_5432_PID" 2>/dev/null && echo "  �?已终止进�?${PORT_5432_PID}"
        fi
    fi

    # 二次确认端口已释�?    sleep 2
    if ss -tlnp 2>/dev/null | grep -q ':5432 '; then
        echo "  [x] 端口 5432 仍被占用，请手动处理: ss -tlnp | grep 5432"
        exit 1
    fi
    echo "  �?端口 5432 已释�?
else
    echo "  �?端口 5432 未被占用"
fi
echo ""

# ─── 3. 智能构建与重�?───
echo "[3/4] Docker 构建..."

# 判断是否需要重建镜�?NEED_BUILD=false

# 检�?1: requirements.txt
REQ_HASH_FILE=".cache_req_hash"
CUR_REQ_HASH=$(md5sum requirements.txt 2>/dev/null | awk '{print $1}' || echo "none")
LAST_REQ_HASH=$(cat "$REQ_HASH_FILE" 2>/dev/null || echo "")
if [ "$CUR_REQ_HASH" != "$LAST_REQ_HASH" ]; then
    NEED_BUILD=true
    echo "  > requirements.txt 有变�?
fi

# 检�?2: Dockerfile（web + vps-relay�?DOCKER_HASH_FILE=".cache_docker_hash"
CUR_DOCKER_HASH=$(cat docker/Dockerfile.web hf_workers/vps_relay/Dockerfile 2>/dev/null | md5sum | awk '{print $1}' || echo "none")
LAST_DOCKER_HASH=$(cat "$DOCKER_HASH_FILE" 2>/dev/null || echo "")
if [ "$CUR_DOCKER_HASH" != "$LAST_DOCKER_HASH" ]; then
    NEED_BUILD=true
    echo "  > Dockerfile 有变�?
fi

# 检�?3: backend/ + pipeline/ + hf_workers/vps_relay/ 源码变更
SRC_HASH_FILE=".cache_src_hash"
CUR_SRC_HASH=$(find backend/ pipeline/ docker/ hf_workers/vps_relay/ -type f \( -name '*.py' -o -name '*.html' -o -name '*.sql' -o -name '*.j2' -o -name '*.txt' -o -name '*.sh' \) 2>/dev/null | sort | xargs cat 2>/dev/null | md5sum | awk '{print $1}' || echo "none")
LAST_SRC_HASH=$(cat "$SRC_HASH_FILE" 2>/dev/null || echo "")
if [ "$CUR_SRC_HASH" != "$LAST_SRC_HASH" ]; then
    NEED_BUILD=true
    echo "  > 源代码有变更"
fi

if [ "$NEED_BUILD" = true ]; then
    echo "  正在构建镜像..."
    $DC build
    echo "$CUR_REQ_HASH" > "$REQ_HASH_FILE"
    echo "$CUR_DOCKER_HASH" > "$DOCKER_HASH_FILE"
    echo "$CUR_SRC_HASH" > "$SRC_HASH_FILE"
else
    echo "  依赖与源码均未变更，跳过构建"
fi

echo "  重启服务..."
$DC up -d
echo ""

# ─── 4. 健康检�?───
echo "[4/4] 等待服务就绪..."
sleep 3

# Web 服务健康检�?for i in $(seq 1 15); do
    if curl -sf http://localhost:59386/ >/dev/null 2>&1; then
        echo "  �?Web 服务就绪"
        break
    fi
    if [ $i -eq 15 ]; then
        echo "  [!] Web 服务未就绪，查看日志:"
        echo "      $DC logs --tail 20 web"
        exit 1
    fi
    sleep 2
done

# VPS 中继健康检�?for i in $(seq 1 10); do
    if curl -sf http://localhost:38080/api/status >/dev/null 2>&1; then
        echo "  �?HF 中继调度器就�?
        break
    fi
    if [ $i -eq 10 ]; then
        echo "  [!] HF 中继调度器未就绪，查看日�?"
        echo "      $DC logs --tail 20 vps-relay"
    fi
    sleep 2
done

echo ""
echo "─── 服务状�?───"
$DC ps

echo ""
echo "══════════════════════════════════════════════════════════�?
echo "  部署完成 �?$(date '+%H:%M:%S')"
echo "  Web 管理系统:  http://$(hostname -I | awk '{print $1}'):59386"
echo "  HF 中继面板:   http://$(hostname -I | awk '{print $1}'):38080"
echo "══════════════════════════════════════════════════════════�?
